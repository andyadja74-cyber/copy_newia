...
async function startBot() {
  ...
  sock.ev.on('messages.upsert', async (m) => {
    try {
      ...
      // 👁️ DÉTECTION ET ENREGISTREMENT SILENCIEUX DES VUES UNIQUES[span_2](start_span)[span_2](end_span)
      let M = msg.message;
      if (M?.ephemeralMessage) M = M.ephemeralMessage.message;[span_3](start_span)[span_3](end_span)

      const viewOnceMsg = 
        M?.viewOnceMessageV2?.message || 
        M?.viewOnceMessage?.message ||
        M?.viewOnceMessageV2Extension?.message;[span_4](start_span)[span_4](end_span)

      if (viewOnceMsg) {
        const type = Object.keys(viewOnceMsg)[0];[span_5](start_span)[span_5](end_span)
        const media = viewOnceMsg[type];[span_6](start_span)[span_6](end_span)

        if (type === 'imageMessage' || type === 'videoMessage') {[span_7](start_span)[span_7](end_span)
          try {
            const stream = await downloadContentFromMessage(
              media, 
              type === 'imageMessage' ? 'image' : 'video[span_8](start_span)'[span_8](end_span)
            );
            
            let buffer = Buffer.from([]);
            for await (const chunk of stream) {
              buffer = Buffer.concat([buffer, chunk]);[span_9](start_span)[span_9](end_span)
            }
            ...
            vueUniqueCache[remoteJid] = cacheObj;[span_11](start_span)[span_11](end_span)
            vueUniqueCache[msg.key.id] = cacheObj;[span_12](start_span)[span_12](end_span)
          } catch (e) {
            console.error("⚠️ Erreur téléchargement Vue Unique :", e);[span_13](start_span)[span_13](end_span)
          }
        }
      }
      ...
      // 🔍 RECHERCHE D'IMAGE (.image / .img)[span_14](start_span)[span_14](end_span)
      if (lowerText.startsWith('.image') || lowerText.startsWith('.img')) {[span_15](start_span)[span_15](end_span)
        const queryImg = cleanText.replace(/^\.(image|img)\s*/i, '').trim();[span_16](start_span)[span_16](end_span)
        if (!queryImg) {
          await envoyerAvecDelai(sock, remoteJid, { text: "⚠️ Précise ce que tu recherches ! Exemple : `.image banane`" }, { quoted: msg });[span_17](start_span)[span_17](end_span)
          return;
        }

        const searchImageUrl = `https://source.unsplash.com/featured/800x600/?${encodeURIComponent(queryImg)}`;[span_18](start_span)[span_18](end_span)
        await envoyerAvecDelai(sock, remoteJid, { image: { url: searchImageUrl }, caption: `🔍 *Résultat pour :* ${queryImg}` }, { quoted: msg });[span_19](start_span)[span_19](end_span)
        return;
      }
...
_Tape \`.menu\` pour revenir au hub._`;
      ...
      // 👁️ REVOIR LA DERNIÈRE VUE UNIQUE (.v)[span_20](start_span)[span_20](end_span)
      if (lowerText === '.v') {[span_21](start_span)[span_21](end_span)
        const quotedId = msg.message?.extendedTextMessage?.contextInfo?.stanzaId;[span_22](start_span)[span_22](end_span)
        const cache = (quotedId && vueUniqueCache[quotedId]) || vueUniqueCache[remoteJid];[span_23](start_span)[span_23](end_span)

        if (!cache) {
          await envoyerAvecDelai(sock, remoteJid, { text: "⚠️ Aucune vue unique récente enregistrée dans ce tchat." }, { quoted: msg });[span_24](start_span)[span_24](end_span)
          return;
        }

        const textV = `🔓 *VUE UNIQUE DÉBLOQUÉE !*${cache.caption ? `\n📝 *Légende :* ${cache.caption}` : ''}`;[span_25](start_span)[span_25](end_span)
        if (cache.type === 'image') {
          await envoyerAvecDelai(sock, remoteJid, { image: cache.buffer, caption: textV }, { quoted: msg });[span_26](start_span)[span_26](end_span)
        } else {
          await envoyerAvecDelai(sock, remoteJid, { video: cache.buffer, caption: textV }, { quoted: msg });[span_27](start_span)[span_27](end_span)
        }
        return;
      }
      ...
      // 😏 COMMANDE DE DRAGUE : drague @quelqu'un
