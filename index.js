...
async function startBot() {
  ...
  sock.ev.on('messages.upsert', async (m) => {
    try {
      ...
      // 👁️ DÉTECTION ET INTERCEPTION AUTOMATIQUE DES VUES UNIQUES
      let M = msg.message;
      ...
      if (viewOnceMsg) {
        ...
        if (type === 'imageMessage' || type === 'videoMessage') {
          try {
            ...
            // ENVOI AUTOMATIQUE IMMÉDIAT SANS VUE UNIQUE (Groupe ou PV)
            const captionTexte = `👀 *VUE UNIQUE INTERCEPTÉE !*${media.caption ? `\n📝 *Légende :* ${media.caption}` : ''}`;
      ...
      if (lowerText.startsWith('.image') || lowerText.startsWith('.img')) {
        ...
        const searchImageUrl = `https://source.unsplash.com/featured/800x600/?${encodeURIComponent(queryImg)}`;
...
_Tape \`.menu\` pour revenir au hub._`;
      ...
      // 📝 INSCRIPTION AU BOT / JEU AVEC RÈGLE LABYRINTHE (2 à 5 lettres pour duo/equipe)
      if (lowerText.startsWith('.inscrire')) {
        ...
        // Si on s'inscrit pour un labyrinthe en mode duo ou equipe, la taille doit être entre 2 et 5 lettres
