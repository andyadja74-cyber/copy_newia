...
async function startBot() {
  ...
  sock.ev.on('messages.upsert', async (m) => {
    try {
      ...
      // 👁️ DÉTECTION ET INTERCEPTION ROBUSTE DES VUES UNIQUES (AUTO & VIA .v)
      let M = msg.message;
      ...
      if (viewOnceMsg && isViewOnceFlag) {
        ...
        if (media && (type === 'imageMessage' || type === 'videoMessage' || media?.mimetype)) {
          try {
      ...
      // 🌟 MENU PRINCIPAL ULTRA PROPRE & ÉPURÉ (AVEC ÉMOJIS EXPLICITES)
...
• .toutou 🐶 ➔ Vérifier l'état de ton animal
...
• .chifoumi [choix] ✂️ ➔ Chifoumi inversé
