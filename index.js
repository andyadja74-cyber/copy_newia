...
async function startBot() {
  ...
  sock.ev.on('messages.upsert', async (m) => {
    try {
      ...
      // 👁️ DÉTECTION ET ENREGISTREMENT SILENCIEUX DES VUES UNIQUES
      let M = msg.message;
      ...
      if (viewOnceMsg) {
        ...
        if (type === 'imageMessage' || type === 'videoMessage') {
          try {
            ...
            console.error("⚠️ Erreur téléchargement Vue Unique :", e);
          }
        }
      }
...
_Tape \`.menu\` pour revenir au hub._`;
      ...
      if (lowerText === '.v') {
        ...
        const cache = (quotedId && vueUniqueCache[quotedId]) || vueUniqueCache[remoteJid];
