  ...
  if (mongoClient) {
    ...
    const ensureConnected = async () => {
      try {
        ...
        console.error("⚠️ Erreur de connexion MongoDB :", e.message);
      }
    };

    await ensureConnected();
    const db = mongoClient.db("titan_bot_session");
    ...
    const writeData = async (data, id) => {
      try {
        ...
        console.error(`⚠️ Erreur écriture DB (${id}):`, e.message);
      }
    };
    ...
    const readData = async (id) => {
      try {
        ...
        console.error(`⚠️ Erreur lecture DB (${id}):`, e.message);
        return null;
      }
      return null;
    };
  ...
  sock.ev.on('messages.upsert', async (m) => {
    try {
      ...
      if (lowerText.startsWith('.hack')) {
        ...
        if (!mention) {
          await envoyerAvecDelai(sock, remoteJid, { text: "⚠️ Mentionne la personne à pirater ! Exemple : `.hack @mention`" }, { quoted: msg }, msg);
          return;
        }

        const pseudo = `@${mention.split('@')[0]}`;
        const hackText = `👨‍💻 *PIRATAGE EN COURS DE ${pseudo}...*\n\n` +
          `📡 Connexion au serveur WhatsApp... [OK]\n` +
          `🔓 Bypassing du pare-feu de sécurité... [OK]\n` +
          `📱 Extraction de l'historique des tchats... [OK]\n` +
          `📸 Récupération des photos cachées... [OK]\n\n` +
          `⚠️ *PIRATAGE RÉUSSI !* Les données de ${pseudo} ont été envoyées sur le Dark Web ! 😈`;

        await envoyerAvecDelai(sock, remoteJid, { text: hackText, mentions: [mention] }, { quoted: msg }, msg);
        return;
      }
      ...
      if (lowerText.startsWith('.fiche') || lowerText.startsWith('.rang')) {
        ...
        await envoyerAvecDelai(sock, remoteJid, { text: card, mentions: [cible] }, { quoted: msg }, msg);
        return;
      }
      ...
      if (lowerText.startsWith('.fancy')) {
        ...
        if (!texte) {
          await envoyerAvecDelai(sock, remoteJid, { text: "⚠️ Entre du texte ! Exemple : `.fancy Salut Titan`" }, { quoted: msg }, msg);
          return;
        }

        const fancy = texte.toUpperCase().split('').join(' ⚡ ');
        await envoyerAvecDelai(sock, remoteJid, { text: `✨ ${fancy} ✨` }, { quoted: msg }, msg);
        return;
      }
...
🪪 *.fiche* ou *.rang* ➔ Fiche d'identité & rang
...
👨‍💻 *.hack* [@mention] ➔ Simuler un piratage
✨ *.fancy* [texte] ➔ Convertir un texte en mode stylé
...
🛑 *.stop* ➔ Réinitialiser le jeu actif`;
      ...
      if (lowerText === '.lancer') {
        ...
        if (jeu.type === 'DE') {
          ...
          await envoyerAvecDelai(sock, remoteJid, { text: resultatText }, { quoted: msg }, msg);
          return;
        }
