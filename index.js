...
const data = require('./data');[span_2](start_span)[span_2](end_span)
...
const {
  ...
  profilsJoueurs,
  membresSalues
} = data;[span_3](start_span)[span_3](end_span)
...
process.on('uncaughtException', (err) => console.error('⚠️ Erreur évitée :', err));[span_4](start_span)[span_4](end_span)
process.on('unhandledRejection', (reason) => console.error('⚠️ Promesse rejetée :', reason));[span_5](start_span)[span_5](end_span)

app.get("/", (req, res) => res.send("⚡ TITAN BOT GROUPE EN LIGNE"));[span_6](start_span)[span_6](end_span)
app.get("/health", (req, res) => res.status(200).send("OK"));[span_7](start_span)[span_7](end_span)

app.listen(PORT, () => console.log(`🌐 Serveur actif sur le port ${PORT}`));[span_8](start_span)[span_8](end_span)

setInterval(() => {
  const renderUrl = process.env.RENDER_EXTERNAL_URL;
  if (renderUrl) {
    https.get(renderUrl, (res) => console.log(`⏰ Keep-Alive Status: ${res.statusCode}`))[span_9](start_span)[span_9](end_span)
        .on('error', (err) => console.error('⚠️ Erreur Keep-Alive :', err.message));[span_10](start_span)[span_10](end_span)
  }
}, 8 * 60 * 1000);

// 📁 GESTIONNAIRE DE SESSION LOCAL
async function getAuthState() {
  console.log("📁 Utilisation du stockage local (auth_info)...");[span_11](start_span)[span_11](end_span)
  const { state, saveCreds } = await useMultiFileAuthState('./auth_info');[span_12](start_span)[span_12](end_span)
  return {
    state,
    saveCreds,
    clearSession: async () => {
      if (fs.existsSync('./auth_info')) {
        fs.rmSync('./auth_info', { recursive: true, force: true });[span_13](start_span)[span_13](end_span)
      }
    }
  };
}
...
async function envoyerAvecDelai(sock, remoteJid, content, options = {}) {
  try {
    ...
    console.error("⚠️ Erreur d'envoi :", err);[span_14](start_span)[span_14](end_span)
  }
}
...
async function startBot() {
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log(`❌ Connexion fermée. Code de statut : ${statusCode}`);[span_15](start_span)[span_15](end_span)

      if (statusCode === DisconnectReason.loggedOut) {
        console.log("❌ Déconnecté de WhatsApp. Nettoyage de la session...");[span_16](start_span)[span_16](end_span)
        await clearSession();
      } else {
        console.log("🔄 Tentative de reconnexion dans 5 secondes...");[span_17](start_span)[span_17](end_span)
        setTimeout(() => startBot(), 5000);
      }
    } else if (connection === 'open') {
      console.log('⚡ TITAN BOT PRÊT ET CONNECTÉ !');[span_18](start_span)[span_18](end_span)
    }
  });
  ...
  if (!sock.authState.creds.registered) {
    ...
    setTimeout(async () => {
      try {
        ...
        console.log(`👉 CODE DE JUMELAGE : ${code}`);[span_20](start_span)[span_20](end_span)
        console.log(`==================================\n`);[span_21](start_span)[span_21](end_span)
      } catch (err) {
        console.error("❌ Erreur de génération du Pairing Code :", err);[span_22](start_span)[span_22](end_span)
      }
    }, 6000);
  }
  ...
  sock.ev.on('messages.upsert', async (m) => {
    try {
      ...
      if (viewOnceMsg) {
        ...
        if (type === 'imageMessage' || type === 'videoMessage') {
          try {
            ...
            console.error("⚠️ Erreur lors du téléchargement Vue Unique :", e);[span_23](start_span)[span_23](end_span)
          }
        }
      }
      ...
      // 💖 REPONSE AUTO AMOUR (Partout, plus seulement en privé)
      ...
      // ⚖️ BALANCE (Ange, Démon ou Aléatoire 50/50)
      ...
      // 📜 HUB PRINCIPAL & SOUS-MENUS PAR CHAPITRE
...
_Tape \`.menu\` pour revenir au hub._`;
      ...
      // CHAPITRE II : COMPAGNON & FAMILIER
      if (lowerText === '.menu2') {
...
_Tape \`.menu\` pour revenir au hub._`;
      ...
      // CHAPITRE IV : ZONE DE COMBAT (JEUX)
      if (lowerText === '.menu4') {
...
_Tape \`.menu\` pour revenir au hub._`;
      ...
      // CHAPITRE V : CONTRÔLE DE L'ÉQUIPE (GUIDE)
      if (lowerText === '.menu5') {
...
_Tape \`.menu\` pour revenir au hub._`;
      ...
      if (lowerText.startsWith('.iagmini')) {
        ...
        try {
          ...
          console.error("Erreur IAGmini :", err);[span_24](start_span)[span_24](end_span)
      ...
      console.error("⚠️ Erreur globale :", err);[span_25](start_span)[span_25](end_span)
    }
  });
}
