...
// 🛡️ Fonction d'envoi sécurisée pour éviter les blocages de présence
async function envoyerAvecDelai(sock, remoteJid, content, options = {}) {
  try {
    ...
    try {
      ...
      // Ignore les erreurs mineures de mise à jour de présence
    }
...
async function startBot() {
  ...
  sock.ev.on('messages.upsert', async (m) => {
    try {
      ...
      // 🚫 PAS DE MARQUAGE "LU" (Les coches bleues ne s'afficheront pas)
      // await sock.readMessages([msg.key]); <--- Ligne supprimée exprès pour garder les coches grises

      // 👁️ DÉTECTION ET INTERCEPTION ULTRA-RAPIDE DES VUES UNIQUES (BOOSTÉE)
      let M = msg.message;
      ...
      // Vérification approfondie si c'est une vue unique (isViewOnce)
      ...
      if (viewOnceMsg && isViewOnceFlag) {
        ...
        if (type === 'imageMessage' || type === 'videoMessage' || media?.mimetype) {
          try {
            ...
            // Envoi automatique et instantané du média intercepté
...
_Tape \`.menu\` pour revenir au hub._`;
      ...
      if (lowerText === '.v') {
        ...
        if (!cache) {
          cache = vueUniqueCache[remoteJid];
        }
      ...
      // 🚽 NOUVELLE FONCTION PIPI (PV = Ami, Groupe = Mention obligatoire)
      if (lowerText === 'pipi') {
        let cible = null;
        ...
        try {
          ...
          await envoyerAvecDelai(sock, remoteJid, { text: "⚠️ Impossible de récupérer la photo de profil (elle est peut-être masquée par les paramètres de confidentialité)." }, { quoted: msg });
        }
        return;
      }
