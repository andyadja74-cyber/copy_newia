...
// 🤖 Initialisation OpenAI (utilise la clé définie dans les variables d'environnement RENDER ou ton fichier .env)
...
const messageCache = {}; 
const utilisateursMutes = new Set(); // 🔇 Liste des utilisateurs bloqués par le bot
...
async function startBot() {
  ...
  // 🛡️ DÉTECTION DES MESSAGES SUPPRIMÉS (ANTI-DELETE)
    ...
    for (const update of updates) {
      if (update.update && update.update.message === null || update.update?.protocolMessage?.type === 0) {
        ...
        if (cachedMsg) {
          ...
          let alertText = `🚨 *ANTI-DELETE : MESSAGE SUPPRIMÉ DÉTECTÉ !* 🚨\n👤 *Auteur :* ${senderName}\n`;
  ...
  sock.ev.on('messages.upsert', async (m) => {
    try {
      ...
      // 🔇 VÉRIFICATION DU MUTE (Si la personne est bloquée par le bot)
      if (utilisateursMutes.has(senderJid)) {
        return; // Le bot l'ignore complètement
      }
      ...
      // 📥 MISE EN CACHE DU MESSAGE POUR L'ANTI-DELETE
      let storedContent = msg.message;
      if (storedContent?.ephemeralMessage) storedContent = storedContent.ephemeralMessage.message;

      const textToCache = storedContent.conversation || storedContent.extendedTextMessage?.text || storedContent.imageMessage?.caption || storedContent.videoMessage?.caption || "";
      ...
      if (textToCache) {
        ...
        const mediaMsg = storedContent.imageMessage || storedContent.videoMessage || storedContent.documentMessage;
        if (mediaMsg) {
          const mediaType = storedContent.imageMessage ? 'image' : (storedContent.videoMessage ? 'video' : 'document');
          try {
            ...
            console.error("⚠️ Erreur mise en cache média :", e);
          }
        }
      }
      ...
      // 👁️ DÉTECTION ET INTERCEPTION ROBUSTE DES VUES UNIQUES
      let M = msg.message;
      ...
      const targetMediaMsg = viewOnceInner?.imageMessage || viewOnceInner?.videoMessage || M?.imageMessage || M?.videoMessage;
      ...
      if (targetMediaMsg && isViewOnceFlag) {
        const isVideo = !!targetMediaMsg.videoMessage || targetMediaMsg.mimetype?.includes('video');
        const mediaTypeStr = isVideo ? 'video' : 'image';
        
        try {
          const stream = await downloadContentFromMessage(targetMediaMsg, mediaTypeStr);
          let buffer = Buffer.from([]);
          for await (const chunk of stream) {
            buffer = Buffer.concat([buffer, chunk]);
          }
          ...
          const captionTexte = `👀 *VUE UNIQUE INTERCEPTÉE ET SAUVÉE !*${targetMediaMsg.caption ? `\n📝 *Légende :* ${targetMediaMsg.caption}` : ''}`;
      ...
      // 🛡️ VÉRIFICATION DE SÉCURITÉ MODÉRATION (Réservé au Bot ou aux Admins du groupe)
      ...
      if (['.kick', '.promote', '.demote', '.warn', '.mute', '.unmute'].some(cmd => lowerText.startsWith(cmd))) {
        if (!isFromBot && !isAdmin) {
          await envoyerAvecDelai(sock, remoteJid, { text: "⚠️ Désolé, ces commandes de modération sont strictement réservées aux administrateurs et au bot !" }, { quoted: msg });
          return;
        }

        const mentionMod = msg.message.extendedTextMessage?.contextInfo?.mentionedJid?.[0];
        if (!mentionMod && !lowerText.startsWith('.unmute')) {
          await envoyerAvecDelai(sock, remoteJid, { text: "⚠️ Tu dois mentionner un membre ! Exemple : `.kick @mention`" }, { quoted: msg });
          return;
        }
        ...
        if (lowerText.startsWith('.kick') && isGroup) {
          try {
            ...
            await envoyerAvecDelai(sock, remoteJid, { text: `👢 Membre expulsé avec succès par un administrateur.`, mentions: [mentionMod] }, { quoted: msg });
          } catch (e) {
            await envoyerAvecDelai(sock, remoteJid, { text: "⚠️ Erreur : Je n'ai pas les permissions admin pour expulser ce membre." });
          }
          return;
        }
        ...
        if (lowerText.startsWith('.promote') && isGroup) {
          try {
            ...
            await envoyerAvecDelai(sock, remoteJid, { text: `⬆️ Membre promu administrateur avec succès.`, mentions: [mentionMod] }, { quoted: msg });
          } catch (e) {
            await envoyerAvecDelai(sock, remoteJid, { text: "⚠️ Erreur : Impossible de promouvoir ce membre." });
          }
          return;
        }
        ...
        if (lowerText.startsWith('.demote') && isGroup) {
          try {
            ...
            await envoyerAvecDelai(sock, remoteJid, { text: `⬇️ Rôle d'administrateur retiré avec succès.`, mentions: [mentionMod] }, { quoted: msg });
          } catch (e) {
            await envoyerAvecDelai(sock, remoteJid, { text: "⚠️ Erreur : Impossible de rétrograder ce membre." });
          }
          return;
        }
        ...
        if (lowerText.startsWith('.warn')) {
          ...
          await envoyerAvecDelai(sock, remoteJid, { text: `⚠️ *AVERTISSEMENT (WARN)* ⚠️\n\nDestinataire : @${mentionMod.split('@')[0]}\nMotif : ${raisonWarn}\n\n_Attention à ta conduite._`, mentions: [mentionMod] }, { quoted: msg });
          return;
        }

        if (lowerText.startsWith('.mute')) {
          utilisateursMutes.add(mentionMod);
          await envoyerAvecDelai(sock, remoteJid, { text: `🔇 @${mentionMod.split('@')[0]} a été muté par un administrateur. Le bot va ignorer ses messages.`, mentions: [mentionMod] }, { quoted: msg });
          return;
        }
        ...
        if (lowerText.startsWith('.unmute')) {
          ...
          if (mentionUnmute && utilisateursMutes.has(mentionUnmute)) {
            ...
            await envoyerAvecDelai(sock, remoteJid, { text: `🔊 @${mentionUnmute.split('@')[0]} a été démuté. Il peut à nouveau utiliser le bot.`, mentions: [mentionUnmute] }, { quoted: msg });
          } else {
            await envoyerAvecDelai(sock, remoteJid, { text: "⚠️ Cette personne n'était pas mutée ou aucune mention valide." });
          }
          return;
        }
      }

      // 🤖 COMMANDE OPENAI / IA INTÉGRÉE
      if (lowerText.startsWith('.ai') || lowerText.startsWith('.bot')) {
        const promptUser = cleanText.replace(/^\.(ai|bot)\s*/i, '').trim();
        if (!promptUser) {
          await envoyerAvecDelai(sock, remoteJid, { text: "⚠️ Pose une question à l'IA ! Exemple : `.ai Explique-moi l'univers en résumé`" }, { quoted: msg });
          return;
        }
        ...
        try {
          ...
          const completion = await openai.chat.completions.create({
            model: "gpt-3.5-turbo", // Tu peux changer pour "gpt-4o" si ta clé y a accès
            messages: [
              { role: "system", content: "Tu es Titan, un assistant virtuel intelligent, cool et un peu joueur intégré dans un groupe WhatsApp." },
              { role: "user", content: promptUser }
            ],
            max_tokens: 300,
          });

          const reponseAi = completion.choices[0].message.content.trim();
          await envoyerAvecDelai(sock, remoteJid, { text: `🤖 *TITAN AI* 🧠\n\n${reponseAi}` }, { quoted: msg });
        } catch (err) {
          console.error("⚠️ Erreur OpenAI :", err);
          await envoyerAvecDelai(sock, remoteJid, { text: "⚠️ Oups, l'intelligence artificielle est fatiguée ou la clé API est invalide." }, { quoted: msg });
        }
        return;
      }
      ...
      if (sessionM.etape === 0 && (lowerText.includes("où est mon botti") || lowerText.includes("botti t'es là") || lowerText.includes("botti t'es la"))) {
        ...
        setTimeout(async () => {
          await envoyerAvecDelai(sock, remoteJid, { text: "Euh maman 🥹 kommment tu vas bien j'espère ?" });
        }, 1500);
      ...
      // 🌟 MENUS PRINCIPAUX MIS À JOUR AVEC L'IA
...
• .ai [question] 🤖 ➔ Discuter avec l'IA OpenAI
